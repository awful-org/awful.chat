<script lang="ts">
   ...
   * preview and device pickers /qc's green room (DeviceCheck) uses before
   * you ever join. Picking a camera here saves it as the default
   * (DeviceCheck.pickCamera already does that), the same way the screen
   * share quality picker remembers its choice.
   *
...
<Dialog bind:open={uiState.cameraPickerOpen}>
  ...
  >
    ...
    <div class="flex flex-col gap-3 px-4 pb-4">
      <DeviceCheck open={uiState.cameraPickerOpen} />
    </div>
